...
class Handlers {
    ...
    */
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        if (oSession.fullUrl.Contains("createMatchOrder?lid2")) {

            var requestBody = oSession.GetRequestBodyAsString();
            var filePath = "F:\\抢票\\spider_img\\request.json"; // 更改为你的文件路径
            System.IO.File.AppendAllText(filePath, Environment.NewLine + requestBody);
            if (oSession.oRequest.headers.Exists("Authorization")) {
                var authHeader = oSession.oRequest.headers["Authorization"];
                var filePath = "F:\\抢票\\spider_img\\auth.json"; // 更改为你的文件路径
        ...
        if (oSession.fullUrl.Contains("wx_api/v1/Login/index")) {
            var requestBody = oSession.GetRequestBodyAsString();
            var filePath = "F:\\抢票\\spider_img\\index.json"; // 更改为你的文件路径
            System.IO.File.WriteAllText(filePath, requestBody);

        }

        if (oSession.fullUrl.Contains("Member/getCrewsList?lid2=")) {
            var responseBody = oSession.GetResponseBodyAsString();
            var filePath = "F:\\抢票\\spider_img\\member.json"; // 更改为你的文件路径
            System.IO.File.WriteAllText(filePath, responseBody);
